...
class GalonTransaksiListAdapter (private val context: Context, private val galonItemList: List<CartItem>, private val listener: OnItemClickListener) : RecyclerView.Adapter<GalonTransaksiListAdapter.GalonItemViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): GalonItemViewHolder {
        val binding: ListGalonTransaksiBinding = ListGalonTransaksiBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    class GalonItemViewHolder(var binding: ListGalonTransaksiBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(cart: CartItem, position: Int, listener: OnItemClickListener) {
            Picasso.get().load(cart.galon.image).placeholder(R.drawable.ic_launcher_background).into(binding.imageGalonTransaksi)
            binding.textNamaGalonTransaksi.text = cart.galon.merk
            binding.textHargaGalonTransaksi.text = "Rp. " + cart.galon.hargaJual
